...
//			string selectedName = dropdown.options[dropdown.value].text;
			try {
				...
				// ReadPedometer(now);
				Debug.Log("Reading Quantity Data ");
				ReadQuantityData(HKDataType.HKQuantityTypeIdentifierHeartRate, start, now);
				Debug.Log(start);
				Debug.Log(now);
				ReadQuantityData(dataType, start, now);
				HKDataType dataType2 = HKDataType.HKQuantityTypeIdentifierHeartRate;
				ReadQuantityData(dataType2, start, now);
